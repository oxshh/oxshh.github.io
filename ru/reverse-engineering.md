...
### 1) Анализируем дизассемблированный код
...
### 2) Восстанавливаем структуру пикселя
...
- Byte3 ∈ `[0x20; 0x7e]` (печатаемый ASCII)
    
- Byte3 ≠ `0x20` (иначе не выполнится условие `rsi_4 == 1650`)
    

То есть пиксель должен быть:

Code

```
8c 1d 40 XX   где XX — любой печатаемый символ, кроме 0x20
```

Мы выбрали `0x2e` (`.`).

### 3) Проверяем финальное условие вызова win()
...
- `0x672 = 1650` — количество пикселей.
    
- `rsi_4` — число пикселей, у которых 4-й байт != 0x20.
    
- `rdx_2` — флаг, что первые три байта всех пикселей корректны.
    

Чтобы попасть в `win()`:

- ВСЕ пиксели должны иметь корректные первые три байта.
    
- ВСЕ пиксели должны иметь 4-й байт ≠ 0x20.
    

### 4) Строим корректный файл

Мы собираем:

- Заголовок (24 байта)
    
- 1650 пикселей по 4 байта
